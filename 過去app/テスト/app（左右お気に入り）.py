...
# -------------------------------------------------
# 検索結果（2ペイン：左=全件/右=お気に入り）＋リンク化＋即時反映
# -------------------------------------------------
st.markdown("### 検索結果")
...
if "llm_keywords" in all_cols_full:
    ...
    hide_cols_full.update(all_cols_full[start_full:])  # llm_keywords 以降すべて非表示
...
# ===== 2ペイン表示 =====
col_main, col_fav = st.columns([2.2, 1.3])
...
with col_main:
    ...
    # --- サブセット差分適用：表示中の行だけ更新、他は保持 ---
    old_favs = set(st.session_state.favs)
    subset_ids_main = set(disp["_row_id"].tolist())  # ← 今まさに表示中のID集合
    checked_subset_main = set(
        edited_main.loc[edited_main["★"] == True, "_row_id"].tolist()
    )

    new_favs_after_main = (old_favs - subset_ids_main) | checked_subset_main
# 右：お気に入り一覧（★編集可）— フィルター無視（df 全体）で表示
with col_fav:
    st.subheader(f"⭐ お気に入り（{len(st.session_state.favs)} 件）")

    # 全体から可視列で表を作り、_row_id を付与
    fav_disp_full = df[visible_cols_full].copy()
    fav_disp_full["_row_id"] = fav_disp_full.apply(make_row_id, axis=1)

    # 現在のお気に入りIDに該当する行だけ抽出（フィルターは無視）
    fav_disp = fav_disp_full[fav_disp_full["_row_id"].isin(st.session_state.favs)].copy()

    if fav_disp.empty:
        st.info("お気に入りは未選択です。左表の『★』にチェックしてください。")
        fav_edited = None
    else:
        ...
        fav_edited = st.data_editor(
            ...
            disabled=[c for c in fav_display_order if c != "★"],  # ★のみ編集可
            height=520,
            num_rows="fixed",
        )

# 反映ロジック（サブセット差分適用：表示外のお気に入りは保持）
# 左ペイン適用後の集合をベースに
new_favs = set(new_favs_after_main)

# 右ペインがある場合、右の表示サブセット（fav_disp）で上書き
if fav_edited is not None:
    subset_ids_fav = set(fav_disp["_row_id"].tolist())
    fav_checked_subset = set(fav_edited.loc[fav_edited["★"] == True, "_row_id"].tolist())
    new_favs = (new_favs - subset_ids_fav) | fav_checked_subset

if new_favs != set(st.session_state.favs):
    st.session_state.favs = new_favs
    st.rerun()

# 下部アクション（CSV出力など）
c1, c2 = st.columns([1, 2])
with c1:
    st.caption(f"現在のお気に入り：{len(st.session_state.favs)} 件")
with c2:
    # メイン画面と同じ列だけCSV（★と内部IDは除く）
