...
# ===== 縦積み表示：上=全件（編集可）、下=お気に入り（編集可・フィルター無視） =====
...
# 下：お気に入り（★編集可・フィルター無視 = df 全体から作る）
st.subheader(f"⭐ お気に入り（{len(st.session_state.favs)} 件）")

# 右ペイン用：フィルター無視の全体可視列（まだ無ければ上で定義しておく）
all_cols_full = list(df.columns)
hide_cols_full = {"相対PASS", "終了ページ", "file_path", "num_pages", "file_name"}
if "llm_keywords" in all_cols_full:
    start_full = all_cols_full.index("llm_keywords")
...
if fav_edited is not None:
    ...
    fav_checked_subset = set(
        fav_edited.loc[fav_edited["★"] == True, "_row_id"].tolist()
    )
